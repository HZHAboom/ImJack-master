...
//                // 打开在列表中发现的红包
    ...
    private boolean hasImageViewChild(AccessibilityNodeInfo node){
        return node.getChildCount() == 2 && "android.widget.ImageView".equals(node.getChild(0).getClassName().toString()) && "android.widget.ImageView".equals(node.getChild(1).getClassName().toString());
    }
}
